...
app.get('/appointment/all', async (req, res) => {
    try {
        ...
        const db = conn.db('MSWD');
        const bookappointments = db.collection('bookappointment');
        const allAppointments = await bookappointments.find({}).toArray();
        conn.close();
...
// Assuming the required modules and setup are already in place
...
app.post('/uploaddp', async function(req, res){
    try
    {
        ...
        myfile.mv('../src/images/photo/'+ fname +'.jpg', function(err){
            ...
            res.json("File uploaded successfully...");7
        });
        conn = await client.connect();
        db = conn.db('MSWD');
        users = db.collection('users');
        ...
        conn.close();
        res.status(200).json({ success: true, avatarUrl: fname + '.jpg' });
    }catch(err)
    {
        res.json(err).status(404);
    }
